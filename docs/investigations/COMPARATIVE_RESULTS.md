...
non-cleanup scenario; commit `2e5dadc`). Its temporal column now comes from a
...
For Diamorphine, Timeline is applicable only to the loading and taint facets of
D01/D02, so both results are `P`; it cannot observe runtime hiding, the
procfs/sysfs discrepancy, the syscall table, or ftrace state. D01/D02 are
mechanically `C` under the target-level contract because Timeline and Memory
both find a facet. The narrower evidentiary claim is that only D02's taints are
the same attribute corroborated across independent acquisitions: disk
`kern.log`/journal and memory `modxview`. Timeline's 100% and Memory's 50% have
different applicability sets and are not a performance ranking.
